...
function listOrderDetails(data, cb){ //cb is representing the functionality of caller
    var sql = `SELECT *
               FROM ecommercedb.Users U INNER JOIN ecommercedb.OrderDetails O 
               ON U.ID = O.UserID 
               INNER JOIN ecommercedb.OrderItems OI 
               ON O.ID = OI.OrderID 
               INNER JOIN ecommercedb.Products P 
               ON P.ID = OI.ProductID WHERE U.ID = ?`;
...
function editOrder(data,cb){
    ...
    var values =[];
    if(data.payment){
        var sql = `UPDATE OrderDetails SET OrderStatus =?,
                   UpdatedAt =now() WHERE ID =?`;
        values.push(2);
    }
    else{
...
module.exports = {listOrderDetails, getOrderDetails,  findOrderByUser, addOrder, editOrder}